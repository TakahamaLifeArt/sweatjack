...
//	$('.pop_size', '#step2').on('click', function(){
	...
	$('#pop_pack').click( function(){
		var msg = '<h3>袋詰</h3><hr>';
		msg += '<p><img width="100%" alt="袋詰あり" src="./img/img_packing.jpg"><img alt="袋詰なし" src="./img/img_nopacking.jpg"></p>';
	...
	*	カテゴリー変更でアイテム一覧表示を更新
	*
	*	2013-12-06 Sweatjack用に変更
	* 	2016-09-21 カテゴリにTシャツとブルゾンを追加
	*/
...
//	$('#estimation_wrap tbody input[type="number"]').on( 'change', function(){
	...
	*/
	...
	/********************************
	*	デザインの備考、インク色指定、デザインテンプレートのテキストエリア
	*/
...
//	});
	...
	*/
	$('#member_login').on('click', function(){
			...
			$.ajax({url:'user_login.php', type:'get', dataType:'json', async:false, data:{'login':'true', 'email':$('#login_input_email').val(),'pass': $('#login_input_pass').val(), 'reg_site':'5'}, 
				success: function(r){
					if(r.length!=0){
...
*/
	...
	jQuery.extend({
		...
		*/
		...
		category: {
			't-shirts': {'id':1, 'name':'Tシャツ'},
			'sweat': {'id':2, 'name':'スウェット'},
			'outer': {'id':6, 'name':'ブルゾン'},
		},
		init: function(){
		/*
		*	initialize
		*/
			...
			*/
				...
				$.itemparam.categoryname = 'スウェット・パーカー';
				$.itemparam.itemid = 173;
				$.itemparam.itemcode = '185-nsz';
				$.itemparam.itemname = 'スタンダードジップパーカー';
			}

			$.itemparam.colorname = 'ターコイズ';
...
//			$('select', '#pos_wrap').uniform({fileDefaultText:''});
		...
		*/
			var isInks = false;
			if($('#pos_wrap .inkbox select').length>0 && !$('#noprint').is(':checked')){
				$('#pos_wrap .inkbox select').each( function(){
					if($(this).val()!=0){
						isInks = true;
						return false;	// break;
		...
		*/
			...
			if($('#pass').is(':visible')) {
				$.ajax({url:'/php_libs/orders.php', type:'get', dataType:'json', async:false, data:{'act':'checkemail','email':$("#email").val(), 'reg_site':'5'}, 
					success: function(r){
						if(r.length!=0){
							var msg = '<h1>登録済みのメールアドレスです！</h1>';
