...
$(function () {
			...
			success: function (res) {
				...
				for (var s = 0; s < x.length; s++) {
					...
					if ($.itemparam.categorykey == 't-shirts') {
						if (code == '085-cvt') tmp[0] = {
							'085-cvt': v
						};
						if (code == '300-act') tmp[1] = {
							'300-act': v
						};
						if (code == '5806') tmp[2] = {
							'5806': v
	...
	$('#step2').on('click', '.color_thumb li img', function () {
		...
		$(this).closest('.thumb_wrap').find('.item_image img').attr({
			'src': imgname,
			'alt': code
		});
	...
	$('#add_item_color').click(function () {
		...
		clone.find('.item_image img').attr({
			'src': imgname,
			'alt': code
		});
		...
		setCustomer2: function (args) {
			...
			$.ajax({
				...
				if (typeof me.id == 'undefined') {
					...
					$('#delivery_customer').append($('<option>').attr({
						value: '-1'
					}).text('●ご住所'));
					if (typeof me.delivery != 'undefined') {
						for (var i = 0; i < me.delivery.length; i++) {
							$('#delivery_customer').append($('<option>').attr({
								value: me.delivery[i].id
							}).text('●お届先' + (i + 1)));
						}
					}
