...
require_once $_SERVER['DOCUMENT_ROOT'].'/../app-def/jd/japaneseDate.php';
...
*/
	...
	*/
	public function update($mode){
		...
		*				  'options': {'discount':割引, 'carriage':送料, 'codfee':代引手数料, 'conbifee':コンビニ手数料,pack:袋詰代},
		...
		*/
			/*
			if($_SESSION['orders']['options']['noprint']==1){
			// 全アイテムのプリント情報を初期化
				foreach($_SESSION['orders']['items'] as $catid=>$v1){
					foreach($v1['item'] as $itemid=>$v2){
						foreach ($v2['design'] as $base => $value) {
							$_SESSION['orders']['items'][$catid]['item'][$itemid]['design'][$base][0]['ink'] = 0;
							for($i=1; $i<count($value); $i++){
								unset($_SESSION['orders']['items'][$catid]['item'][$itemid]['design'][$base][$i]);
							}
						}
					}
				}
		...
		*			   ['noprint']			0:プリントあり(default)		1:プリントなし
		*
		*	return		{'discount':割引, 'carriage':送料, 'codfee':代引手数料, 'conbifee':コンビニ手数料, pack:袋詰代, discountname:[...]}
		*/
	...
	*	p11 追加料金
	*
	*	return			{'discount':割引, 'carriage':送料, 'codfee':代引手数料,'conbifee':コンビニ手数料, pack:袋詰代, discountname:[...], プリントの有無
...
//			$d = explode('-', $opt['deliveryday']);
					...
					for($i=0; $i<count($files[$posid]['img']); $i++){
						...
						if(!isset($val2['design'][$base])){
							if(isset($design[$posid][$base])){
								...
								$_SESSION['orders']['items'][$catid]['item'][$itemid]['design'][$base][0] = $tmp;
							}
						}else{
	...
	*/
	public function reqConfirmation(){
		...
		$user['note_template'] = nl2br($user['note_template']);
	...
	*/
	public function getCategoryInfo($itemid){
	...
	*/
		...
		foreach($files as $posid=>$v){
			...
			for($i=0; $i<count($v['img']); $i++){
				...
				$res .= '<tfoot style="display: none;"><tr><td>'.$ppData['category'].'</td><td>'.$ppData['item'].'</td><td>'.$posname_key.'</td></tr></tfoot>';
				$res .= '<tbody>';
